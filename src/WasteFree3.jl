...
function SMC(N,M;model,λ,ϵ,α,method,mass_mat="adaptive",printl=false)
    T = length(λ)
    X = Array{Matrix,1}(undef,T);
    X[1] = zeros(N,model.D)
    logW = zeros(N,T)
    W = zeros(N,T)
    P = div(N,M)
    ...
    exp_time  = 0.0
    for t = 2:T
        t1 = @timed x,v,u0,u,ke,h = get_particles(X[t-1],W[:,t-1],M,P,ϵ=ϵ[t-1],model=model,λ=λ[t-1],method=method,mass_mat=mass_mat)
        exp_time += t1.time - t1.gctime
        if printl
            println("the new temperature at iteration $(t) is ",λ[t])
        end
        X[t] = merge_array(x)
        H[:,t] = reshape(h,:,1)
        logW[:,t] = get_weights(x,v,u0,u,ke,λ[t-1],λ[t],method=method)
        W[:,t] = getW(logW[:,t])
    end
    return (X=X,λ=λ,W=W,logW=logW,explore_time = exp_time,H=H)
end
end
